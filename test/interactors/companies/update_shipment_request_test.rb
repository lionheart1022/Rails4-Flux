...
class Companies::UpdateShipmentRequestTest < ActiveSupport::TestCase
  ...
  test "run with propose event" do
    shipment_request = create_shipment_request

    assert_emails 1 do
      run_interactor(shipment_request, state: ShipmentRequest::States::PROPOSED)
    end

    assert_match /RFQ proposed/i, ActionMailer::Base.deliveries.last.subject
  end
  ...
  test "run with book event" do
    ...
    assert_emails 1 do
      run_interactor(shipment_request, state: ShipmentRequest::States::BOOKED)
    end
  ...
  test "run with cancel event" do
    ...
    assert_equal @customer_user.email, ActionMailer::Base.deliveries.last.to.first
  end
  ...
  def run_interactor(shipment_request, state: nil)
    ...
    interactor = Companies::ShipmentRequests::Update.new(interactor_params)
    interactor.run
  end
end
