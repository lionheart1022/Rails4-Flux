class GLSFeedbackSpecialFile < CarrierFeedbackFile
  def header_label
    "GLS"
  end
  ...
  def parse_file_and_persist_updates!
    ...
    parser.rows.each do |row|
      ...
      matching_packages =
        ...
        .order(id: :desc)
        .select { |package| package.shipment.carrier_product.get_credentials[:customer_id] == configuration.account_details['customer_id'] }
