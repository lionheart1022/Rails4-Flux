...
class GLSDailySpecial
  ...
  def new_files
    ...
    ftp.nlst.each do |filename|
      match = %r{\A(?<account_number>\d{5})b\.(?<timestamp>.+)\z}.match(filename)
  ...
  NewFile = Struct.new(:configuration, :filename, :timestamp) do
    def feedback_file_relation
      GLSFeedbackSpecialFile.where(configuration: configuration, company: configuration.company, original_filename: filename)
    end
  end
end
