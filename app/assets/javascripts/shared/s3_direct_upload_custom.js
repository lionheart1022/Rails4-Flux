  ...
  $.fn.S3Uploader = function(options) {
      ...
      return $uploadForm.fileupload({
        ...
        progress: function(e, data) {
          ...
          if (callback_url) {
            ...
            // custom, also send file description with request
            content.file_description = $('#file_description').val();
            content.file_is_private = $('#file_is_private').prop('checked');
    ...
    build_content_object = function($uploadForm, file, result) {
      ...
      if (result) {
        ...
        // Custom
        var filepath_without_bucket = $uploadForm.find('input[name=key]').val().replace('/${filename}', '');
        content.filepath = '/' + $uploadForm.data("bucket-name") + '/' + filepath_without_bucket;
        content.url = domain + filepath_without_bucket;
